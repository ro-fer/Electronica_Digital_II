...
Ejercicio 4:
Implementar un código C en el cual al enviar la cadena de caracteres “turn_on_led” a través de la consola el microcontrolador prenda el led integrado en la placa Blue Pill durante 5 segundos. Implementar tanto la recepción (RX) como la transmisión (TX) de la UART utilizando polling.
...
int main(void)
{
	...
	DEVMAP->RCC.REGs.APB2ENR |= (1 << 4);					// Enable GPIOA clock.
	...
	DEVMAP->GPIOs[GPIOC].REGs.CRH = 0; // Limpio todos los bits.
	DEVMAP->GPIOs[GPIOC].REGs.CRH |= (3 << 20); // GPIO pin 13 como salida

	systick_config();

	DEVMAP->GPIOs[GPIOC].REGs.ODR = (1<<13);

	char str[] = "turn_led_on";
	while(1) {
		int full_string = 1;
		for(int i = 0; str[i] != '\0'; i++){
			...
			if(c!=str[i]) {
				full_string = 0;
				USART1_send_string("\n\tEscribiste cualquiera man\n");
				break;
			}
		}
		if(full_string) {
			USART1_send_string("\n\tSe prende\n");
            DEVMAP->GPIOs[GPIOC].REGs.ODR = (0<<13);
			delay_ms(5000);
			USART1_send_string("\n\tSe apaga\n");
            DEVMAP->GPIOs[GPIOC].REGs.ODR = (1<<13);
		}
